...
import { X, Users, Check, Radio } from 'lucide-react';
...
const VideoCall: React.FC<VideoCallProps> = ({ meetingId, userName, onLeave, isTrainer, userId }) => {
  ...
  useEffect(() => {
    ...
    if (!containerRef.current || !window.JitsiMeetExternalAPI) {
      console.error("Jitsi API script not found. Make sure it's in your index.html");
      return;
    }

    if (jitsiApiRef.current) jitsiApiRef.current.dispose();

    // FIXED: Use meet.jit.si which allows framing on Vercel
    ...
    const options = {
      ...
      configOverwrite: {
        prejoinPageEnabled: false, // NO "Join Now" button, direct entry
        startWithAudioMuted: true,
        disableDeepLinking: true,
        // Enabling these for open entry
  ...
  return (
    <div className="fixed inset-0 bg-[#0A0E1A] flex flex-col z-[100] font-sans">
        ...
        <div className="flex items-center gap-3">
          {isTrainer && (
            <button 
              ...
              className="flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider border border-emerald-500/20"
            >
              <Users size={14} /> Manage ({invitedIds.length})
            </button>
          )}
          <button 
            onClick={onLeave}
            className="bg-red-500/10 text-red-500 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider border border-red-500/20"
          >
            Leave
          </button>
        </div>
      ...
      {/* Video Viewport - Iframe will render here */}
        ...
        {showInviteModal && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-[#111827] w-full max-w-sm rounded-[2rem] p-8 border border-white/10"
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-white text-lg font-bold">Invite Students</h3>
                <button onClick={() => setShowInviteModal(false)} className="text-slate-500"><X size={24} /></button>
              </div>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
                {students.map(student => (
                  ...
                  >
                    ...
                    {invitedIds.includes(student.id) && <Check className="text-emerald-500" size={18} />}
                  </div>
                ))}
              </div>
              ...
              <button 
                ...
                className="w-full mt-8 bg-emerald-500 text-black py-4 rounded-2xl font-black text-xs uppercase tracking-widest"
              >
                Done
              </button>
            </motion.div>
